...
joker.calculate = function(self, card, context)
    if (context.first_hand_drawn or context.lobc_proc_wrath) and not context.blueprint and not G.GAME.servant_triggered then
        ...
        if context.lobc_proc_wrath then
...
joker.loc_vars = function(self, info_queue, card)
    if not card.fake_card and card:check_rounds() >= 2 and ((G.GAME.blind and G.GAME.blind.config and G.GAME.blind.config.blind.key ~= "bl_lobc_mg_wrath") or not G.GAME.blind) then info_queue[#info_queue+1] = {key = 'lobc_magical_girl_temp', set = 'Other'} end
    return {vars = {card.ability.extra.x_mult, 0.6}, key = ((G.GAME.blind and G.GAME.blind.config and G.GAME.blind.config.blind.key == "bl_lobc_mg_wrath") and "j_lobc_servant_of_wrath_alt" or nil)}
end
